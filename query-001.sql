...
from (
    ...
    where strCategory = 'trim' and ysnAssembly = 0 and right(ic.strProductID, 2) in ('10') and (@ProductID = '<ALL>' or ic.strProductID in (select item from dbo.funcSplitList(@ProductID, ',')))
) as a
inner join (
...
where iif(cur10ftPricePerLF = 0, 1,(curXLftPricePerLF-cur10ftPricePerLF)/cur10ftPricePerLF) <= @decMarkup
order by decXLMarkupPerLF
